...
export default function TransactionDetailScreen() {
  ...
  return (
    <OrdernowsScreen
      ...
      badge={`${transaction.type === 'credit' ? copy.credit : copy.debit} / ${copy.detail}`}>
          ...
          <View className="flex-1">
            <View
              ...
              <Text
                ...
                {transaction.type === 'credit' ? copy.credit : copy.debit}
              </Text>
            </View>

            <Text className="mt-4 text-h2 text-foreground uppercase">{formatSignedVnd(displayAmount)}</Text>
            ...
            <Text className="mt-3 text-body text-muted-foreground">
              {groupName || localizeOrdernowsText(transaction.subtitle, language)}
              {venueName ? ` • ${venueName}` : ''}
